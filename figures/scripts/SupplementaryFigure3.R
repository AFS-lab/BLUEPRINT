...
QC_raw<-read.table("../data/SupplementaryFigure3_reads_alignment_data.txt")
...
NonUnique<-NonUnique + geom_hline(yintercept=0.25, color='red', linetype='dashed')
...
load("../data/SupplementaryFigure3_scater_object.RData")

mt_reads<-scater::plotPhenoData(
  BLUEPRINT_Bs_scater_QC,
...
ggsave("../pdfs/SupplementaryFigure3.pdf", plot=last_plot(),height = (6.04*1.5), width=(8.57*1.5))
ggsave("../pngs/SupplementaryFigure3.png", plot=last_plot(),height = (6.04*1.5), width=(8.57*1.5))
